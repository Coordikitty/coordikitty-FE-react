...
import classifyClothesApi from "../../apis/closet/classifyClothesApi";
...
const ClothesAppendModal = ({ modalOpen, handleModalClose }) => {
  ...
  const handleImgFile = async (e) => {
    ...
    try {
      ...
      const res = await classifyClothesApi(formData);
      setTypeLv1(res.large);
      setTypeLv2(res.medium);
      setTypeLv3(res.small);
      setStyle(res.majorStyle);
    } catch (e) {
      console.log("옷을 분류하는데에 실패하였습니다.");
    } finally {
      setIsLoading(false);
    }
          ...
          <Stack spacing={2} direction={"row"}>
            ...
            <Stack width={"100%"} spacing={2}>
              ...
              <ColthesPropertySelector
                ...
                disabled={!typeLv1}
                value={typeLv2}
                handleValue={handleLv2}
              >
                {typeLv1 &&
                  Object.keys(clothesInfo[typeLv1]["sub"]).map((el) => {
                    return (
                      <MenuItem key={el} value={el}>
                        {clothesInfo[typeLv1]["sub"][el]["kr"]}
                      </MenuItem>
                    );
                  })}
              </ColthesPropertySelector>
              ...
              <ColthesPropertySelector
                ...
                disabled={!typeLv2}
                value={typeLv3}
                handleValue={handleLv3}
              >
                {typeLv2 &&
                  Object.keys(clothesInfo[typeLv1]["sub"][typeLv2]["sub"]).map(
                    (el) => {
                      return (
                        <MenuItem key={el} value={el}>
                          {clothesInfo[typeLv1]["sub"][typeLv2]["sub"][el]}
                        </MenuItem>
                      );
                    }
                  )}
...
const ColthesPropertySelector = ({
  ...
  return (
    <FormControl fullWidth size="small">
      ...
      <Select
        id={`clothes-${label}-select`}
        labelId={`clothes-${label}-select-label`}
        label={labelKr}
        value={value}
