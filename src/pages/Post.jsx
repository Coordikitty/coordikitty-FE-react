...
import postLikeApi from "../apis/post/postLikeApi";
import postBookmarkApi from "../apis/post/postBookmarkApi";
...
const Post = () => {
  ...
  const handleLike = async () => {
    const res = await postLikeApi(postData.postId);
    setPostData({
      ...postData,
      isLiked: !postData.isLiked,
      postLike: res.response,
    });
  ...
  const handleBookmark = async () => {
    ...
    const res = await postBookmarkApi(postData.postId);
    console.log("postLikeApi res : ", res);
  };
      ...
      {postData ? (
        ...
        >
          ...
          <Stack direction={"row"} justifyContent={"space-between"}>
            ...
            >
              <Typography variant="h3" paddingTop={"0.2rem"}>
                {postData.postLike}
              </Typography>
            </Button>
            <Stack direction={"row"}>
          ...
          <Stack spacing={1}>
            ...
            <Typography variant="body2" color={"primary"}>
              ...
              <InfoSpan>
                계절 : {seasonInfo.find((el) => el.val === postData.season)?.kr}
              </InfoSpan>
              <InfoSpan>
                스타일 :{" "}
                {styleInfo.find((el) => el.style === postData.style)?.kr}
              </InfoSpan>
              <InfoSpan>
                업로드 날자 : {Date(postData.uploadDate).slice(0, -17)}
              </InfoSpan>
            </Typography>
            <Divider></Divider>
          </Stack>
